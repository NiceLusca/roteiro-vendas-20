  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const stageStats = pipelineEntries?.reduce((acc: Record<string, { total: number; avgTime: number; conversions: number }>, entry) => {
        ...
        // Calculate days in stage from entry dates
        const entryDate = new Date(entry.data_entrada_etapa);
        const currentDate = entry.data_conclusao ? new Date(entry.data_conclusao) : new Date();
        const daysInStage = Math.floor((currentDate.getTime() - entryDate.getTime()) / (1000 * 60 * 60 * 24));
        acc[stageName].avgTime += daysInStage;
        return acc;
      }, {}) || {};
