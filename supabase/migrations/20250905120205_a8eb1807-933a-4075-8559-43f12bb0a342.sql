-- Fase 2: Implementar Regras de Negócio

-- 1. Função para calcular Lead Score automaticamente
...
-- 2. Função para determinar classificação do Lead Score
...
-- 3. Trigger para recalcular Lead Score automaticamente
...
-- 4. Função para validar checklist obrigatório
...
-- 5. Função para calcular datas e SLA de pipeline
...
-- Criar trigger para lead_pipeline_entries
DROP TRIGGER IF EXISTS trigger_calculate_pipeline_dates ON public.lead_pipeline_entries;
CREATE TRIGGER trigger_calculate_pipeline_dates
  BEFORE INSERT OR UPDATE ON public.lead_pipeline_entries
  FOR EACH ROW
  EXECUTE FUNCTION public.calculate_pipeline_dates();

-- 6. Trigger para atualizar status_geral do lead baseado em deals e pipeline
...
DROP TRIGGER IF EXISTS trigger_update_lead_status_from_deals ON public.deals;
CREATE TRIGGER trigger_update_lead_status_from_deals
  AFTER INSERT OR UPDATE ON public.deals
  FOR EACH ROW
  EXECUTE FUNCTION public.update_lead_status_from_deals();

-- 7. Função para criar order automaticamente quando deal é ganha
...
-- Criar trigger para criação automática de orders
DROP TRIGGER IF EXISTS trigger_create_order_from_deal ON public.deals;
CREATE TRIGGER trigger_create_order_from_deal
  AFTER INSERT OR UPDATE ON public.deals
  FOR EACH ROW
  EXECUTE FUNCTION public.create_order_from_deal();