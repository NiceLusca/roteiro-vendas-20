...
BEGIN
  ...
  SELECT id INTO pipeline_id FROM public.pipelines WHERE nome = 'Geral (Primário)' LIMIT 1;
  ...
  (pipeline_id, 'Contato Inicial', 1, 3, 'Realizar primeiro contato', 'Humano', false, null, 
   'Lead qualificado e com interesse demonstrado', 'Contato realizado e necessidades identificadas'),
  (pipeline_id, 'Qualificação', 2, 5, 'Agendar sessão de diagnóstico', 'Agendamento', true, 60,
   'Primeiro contato realizado com sucesso', 'Lead qualificado e sessão agendada'),
  (pipeline_id, 'Apresentação', 3, 7, 'Apresentar proposta personalizada', 'Humano', false, null,
   'Sessão de diagnóstico realizada', 'Proposta apresentada e objeções tratadas'),
  (pipeline_id, 'Negociação', 4, 5, 'Follow-up para decisão final', 'Humano', false, null,
   'Proposta apresentada e interesse confirmado', 'Decisão de compra ou recusa definida'),
  (pipeline_id, 'Fechamento', 5, 3, 'Finalizar processo', 'Humano', false, null,
   'Negociação concluída', 'Cliente fechado ou lead arquivado')
  ON CONFLICT DO NOTHING
  RETURNING id INTO stage_1_id, stage_2_id, stage_3_id, stage_4_id, stage_5_id;

  -- Obter os IDs das etapas criadas
  SELECT id INTO stage_1_id FROM public.pipeline_stages WHERE pipeline_id = pipeline_id AND ordem = 1;
  SELECT id INTO stage_2_id FROM public.pipeline_stages WHERE pipeline_id = pipeline_id AND ordem = 2;
  SELECT id INTO stage_3_id FROM public.pipeline_stages WHERE pipeline_id = pipeline_id AND ordem = 3;
  SELECT id INTO stage_4_id FROM public.pipeline_stages WHERE pipeline_id = pipeline_id AND ordem = 4;
  SELECT id INTO stage_5_id FROM public.pipeline_stages WHERE pipeline_id = pipeline_id AND ordem = 5;
