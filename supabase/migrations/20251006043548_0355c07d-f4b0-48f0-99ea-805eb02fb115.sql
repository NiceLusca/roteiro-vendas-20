-- Fix variable name conflict in calculate_pipeline_dates function
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR OLD.etapa_atual_id IS DISTINCT FROM NEW.etapa_atual_id THEN
    ...
    NEW.data_entrada_etapa := current_timestamp_val;
    NEW.data_prevista_proxima_etapa := current_timestamp_val + (COALESCE(stage_prazo, 7) || ' days')::interval;
  END IF;
  
  -- Calcular tempo em etapa com proteção contra NULL
  IF NEW.data_entrada_etapa IS NOT NULL THEN
    NEW.tempo_em_etapa_dias := EXTRACT(DAY FROM AGE(current_timestamp_val, NEW.data_entrada_etapa))::integer;
  ELSE
    NEW.tempo_em_etapa_dias := 0;
  END IF;
  
  -- Calcular dias em atraso
  IF NEW.data_prevista_proxima_etapa IS NOT NULL AND NEW.data_prevista_proxima_etapa < current_timestamp_val THEN
    NEW.dias_em_atraso := EXTRACT(DAY FROM AGE(current_timestamp_val, NEW.data_prevista_proxima_etapa))::integer;
  ELSE
